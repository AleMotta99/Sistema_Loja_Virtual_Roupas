...
typedef struct Tipo_Roupa{
	

    char Tipo[10], Genero[3], Tecido[10], Cor[20], Marca[10], Tamanho;
    char Tipo_Novo[10],Genero_Novo[3], Tamanho_Novo[2], Tecido_Novo[10], Cor_Nova[20], Marca_Nova[10];
    int  Quantidade, CodigoPeca, Quantidade_Nova, Codigo_Novo;
    float Preco, PrecoFabricacao, Preco_Novo, Preco_Fabricacao_Novo;		
}Roupa;

int Cadastrar_Roupa(Roupa Dados_Roupa[]){
	
	int i, Total_Produtos;
	for(i = 0; i < a;i++){
...
void Consultar_Roupa(Roupa Dados_Roupa[]){
    ...
    while(verifica != 4)
    {
       ...
       switch(verifica){
			case 1:{
				...
				Total_Produtos = Cadastrar_Roupa(Dados_Roupa);
				break;
			}
			case 2:{
